...
func _generate() -> void:
	...
	# place the player and their ball somewhere random in a room
	player.position = level.map_to_world(data[len(data) / 2])
	ball.position = level.map_to_world(data[(len(data) / 2) + 1])
	print("positions")
	print("player")
